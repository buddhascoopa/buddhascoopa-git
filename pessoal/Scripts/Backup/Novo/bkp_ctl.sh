...
${ORACLE_HOME}/bin/sqlplus -s internal << EOF >>$BKP_DIR/${SID}_${DATAHORA}_cfile_dest.log 2>>$BKP_DIR/${SID}_${DATAHORA}_cfile_dest.log
set feed off
set head off
set verify off
set pages 0
...
echo "rcp $BKP_CTL_DEST/${ORACLE_SID}_${DATAHORA}_control01.ctl \$HOSTNAME:/`hostname`/$BKP_CTL_DEST/${ORACLE_SID}_${DATAHORA}_control01.ctl" >> $SCR_COPY_CTL
echo "rcp $BKP_CTL_DEST/${ORACLE_SID}_${DATAHORA}_control02.ctl \$HOSTNAME:/`hostname`/$BKP_CTL_DEST/${ORACLE_SID}_${DATAHORA}_control02.ctl" >> $SCR_COPY_CTL
echo "rcp ${UDUMP_DEST}/${BKP_CTL_TRC} \$HOSTNAME:/`hostname`/${BKP_CTL_DEST}/${ORACLE_SID}_${DATAHORA}_cfile.trc"                            >> $SCR_COPY_CTL
