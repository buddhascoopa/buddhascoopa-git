#!/bin/ksh
#
# arch_clean.sh - Gera lista ou faz limpeza dos Archives
#                 da BD especificada
# 
# Intervencao                   Responsavel Data       Descicao
# ----------------------------- ----------- ---------- ---------------------------------------------------------------------
# Criacao                       Erick CB    25/06/2002
# Alteracao                     Erick CB    25/06/2002
# Alteracao                     Erick CB    26/07/2002 * no rm para eliminar os archives que sofreram compress/gzip
#
# Parametros
# $1 = SID
# $2 = clean|list
...
ps -ef | grep pmon_${ORACLE_SID} | grep -v grep > /dev/null
if [ $? -ne 0 ] ; then
